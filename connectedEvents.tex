...
The final output of the entity expansion operation is a list of entities (normally filtered by a minimum number of appearances since the method brings a much higher number of candidate entities), together with their ranking score and the number of appearances in both the main video and in the collected extra documents. Entities scored high in the final classification are more representative of the context which the original video belongs to. 

$\text{EntityExpansion}\left (subtitles, timePeriod \right )= \left ( \left \{ e_{i}, Freq\left ( subtitles, e_{i} \right ), Freq\left ( extraDocs, e_{i} \right ) \right \} \right )$

In this final set, the most important entities will be better ranked since they have appeared repeatedly in the extra documents. Entities that originally did not have a disambiguation URL can have it now if any of the instances appearing inside the additional documents has obtained one. Finally, entities that were not even mentioned in the original transcripts can be now in the list of relevant items since they have been spotted in the additional documents.
