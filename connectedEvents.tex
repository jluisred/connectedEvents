...
\maketitle
\begin{abstract}
There exist many approaches to tackle the challenge of finding relevant entities to a news event context, but these approaches lack: 
...
%Task like video 
The amount of videos shared on the Web is constantly increasing. From a Semantic Web point of view those video segments need to be annotated with structured information before being linked to others. New possibilities of innovative video services based on the semantic descriptions of the video content and media fragments appear based on this idea. One of the main use cases in LinkedTV is analyzing News videos where various events and their corresponding related issues are depicted. This problem is traditionally addressed by applying audiovisual techniques, like in \cite{ballan2011event}.

Extracting semantic and structured content from a video is complex. Named Entity Recognition (NER) techniques were developed as an essential component of the Information Extraction field. Initially, these techniques focused on: identifying atomic information units in texts, named entities; later on classifying entities into predefined categories (also called context types) by classification techniques; and linking to real world objects using web identifiers (Named Entity Disambiguation). Various tools provide such computations, like AlchemyAPI\footnote{\fontsize{8pt}{1em}\selectfont \url{http://www.alchemyapi.com/}} or DBpedia Spotlight\footnote{\fontsize{8pt}{1em}\selectfont \url{http://spotlight.dbpedia.org/}}. It is possible to align the insights of a video with the time when they appear by applying these techniques over video subtitles (or timed text). 

Approaches like Katsiouli \textit{et al.}~\cite{katsiouli2007} demonstrated that applying named entity recognition techniques in combination wtih domain ontologies on video subtitles achieves decent results in task like video classification. 
...
The Five Ws is a popular concept in information gathering in journalistic reporting. It captures the main aspects of a story or incidence: who, when, what, where, and why~\cite{LiJia2007}. We try to represent the news item in terms of four of those five W's (who is involved in the event, where the event is taking place, what the event is about, and when has happened) in order to generate a query that retrieves documents having to with the same event.This is somewhat like what a normal user would tape in a Web search engine when looking for descriptive documents about the same news item.  
...
We have applied the explained approach over the video \url{http://www.bbc.co.uk/news/world-europe-23339199} from the BBC News Europe site. The main story in this media content is the request of  asylum made by Edward Snowden to the Russian government. In an airport in Moscow, he publicly express his desire to obtain political help while he can find a safe way to reach the Latin American countries offering to harbor him. The time period for which this particular part of the event is relevant goes from 2013-07-06 to 2013-07-17.
...
\begin{itemize}
  \item  The lawyer, behind the case, Anatoly Kucherena, is never mentioned.
  %TODO: in the ?
...
Our preliminary results indicate that we can expand the initial set of found entities with more relevant entities not detected by the named entity recognition. Reinforcing the entities by exploring DBpedia paths along the found named entities occurring in news media leads to a more accurate ranking of important entities but does not introduce any new important relevant entities that were initially missing as such. It did however bring up more related entities useful for additional information in the broader context of an event.
