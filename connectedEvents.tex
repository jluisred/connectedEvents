...
Firstly we extracted named entities from the corresponding subtitles and expanded these by collecting related documents to the news event. 
%This should be mentioning before talking about the expansion, maybe.
The entities obtained from this first pass are typical insufficient. 
%Here you mean named entity expansion?
Secondly we applied a second pass named entity extraction on these documents. 
We then ranked all the entities obtained and filter out those least occurring or identified as less important. 
%First Mention DBPedia here instead in the next paragraph? 
Finally we refined the remaining entities by reinforcing them and finding important predicates between them. Ultimately this leaded to a re-ranking of the entities belonging to this news event. %The idea of connection or adjacency probably here too.
